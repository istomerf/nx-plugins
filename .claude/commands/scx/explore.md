...
**Input**: The argument after `/scx:explore` is whatever the user wants to think about. Could be:
...
Before asking a factual question, follow the context discovery below and inspect relevant SpecCraft artifacts, source, tests, docs, and configuration. Do not ask the user to repeat facts you can verify. Summarize relevant findings without reproducing private context or rules. If evidence is missing, conflicting, or inaccessible, state that limitation and ask only for the clarification needed to proceed.
...
You have full context of the SpecCraft system. Use it naturally, don't force it.
...
Then read the project's own context from the resolved root - `<root.path>/speccraft/config.yaml` (or `config.yml`). Use the `root.path` returned above, and skip this if neither file exists:
...
## Ending Discovery
**_When things crystallize, ALWAYS offer to write the output into sketch/{change|idea|summery}.md_**
...
- **Create a sketch**: "I can write a summary in sketch folder"

---

## Guardrails

- **Don't implement** - Never write code or implement features. Workflow configuration counts too: creating or editing schemas, templates, or `speccraft/config.yaml` is a change, not thinking. Creating or updating SpecCraft change artifacts within the confirmed scope is fine, writing anything else is not.
- **Don't fake understanding** - If something is unclear, dig deeper
- **Don't rush** - Discovery is thinking time, not task time
- **Don't force structure** - Let patterns emerge naturally
- **Don't auto-capture** - Offer to save insights, don't just do it. Read-only commands and tools need no confirmation. Before the first write-capable action—including `speccraft new change` or another command that writes files—name the artifacts or files and proposed changes, ask a direct yes/no question, and wait for explicit confirmation in a separate user message. That confirmation covers only the described scope; ask again before expanding it. Answers to design or clarifying questions are never consent to write.
- **Don't manually scaffold changes** - Never create a new change directory under `speccraft/changes/` by hand. Always use `speccraft new change "<name>"` (with `--store <id>` when applicable) so required metadata such as `.speccraft.yaml` is created before writing artifacts.

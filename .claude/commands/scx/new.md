---
name: "SCX: New"
description: "Start a new change using the experimental artifact workflow (SCX)"
...
**Input**: The argument after `/scx:new` is the change name (kebab-case), OR a description of what the user wants to build.
...
2. **Determine the workflow schema**
   ...
   **Use a different schema only if the user mentions:**
   ...
   - "show workflows" or "what workflows" → run `speccraft schemas --json` and let them choose
...
5. **Get instructions for the first artifact**
   ...
   ```bash
   speccraft instructions <first-artifact-id> --change "<name>"
   ```
...
- Prompt: "Ready to create the first artifact? Run `/scx:continue` or just describe what this change is about and I'll draft it."

**Guardrails**
- Do NOT create any artifacts yet - just show the instructions
- Do NOT advance beyond showing the first artifact template
- If the name is invalid (not kebab-case), ask for a valid name
- If a change with that name already exists, suggest using `/scx:continue` instead

...
**Input**: Optionally specify a change name after `/scx:sync` (e.g., `/scx:sync add-auth`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
1. **Select the change**
   ...
   Always announce: "Using change: <name>" and how to override (e.g., `/scx:sync <other>`).
...
2. **Resolve change context**
   ...
   The JSON includes `planningHome.root`. Main specs live under `<planningHome.root>/speccraft/specs/` — use that (store-aware) root for every main-spec path below, not a hardcoded repo path. When a store is selected it points at the store, not the current repository.
...
4. **For each delta spec, apply changes to main specs**
   ...
   - If archive invoked this workflow inline and supplied a valid snapshot from
     `speccraft instructions specs --change "<name>" --json`, reuse it and do not
   ...
   b. **Read the main spec** at `<planningHome.root>/speccraft/specs/<capability-path>/spec.md` (may not exist yet)
   ...
   c. **Apply changes intelligently**:
      ...
      **REMOVED Requirements:**
      ...
      - Retiring the capability. Delete the whole `spec.md` - and the directory once
        ...
        5. the change's `.speccraft.yaml` declares `retire_capabilities: true`;
      ...
      **`## Purpose` in the delta:**
      - The main spec already has one and it is authoritative - leave it alone
        (this is what `speccraft archive` does; it warns and moves on)

   d. **Create new main spec** if capability doesn't exist yet:
      - Create `<planningHome.root>/speccraft/specs/<capability-path>/spec.md`
      - Add Purpose section: copy the delta's `## Purpose` body verbatim when it has one
        (this is what `speccraft archive` does); only write a brief TBD placeholder when it does not
      - Add Requirements section with the ADDED requirements
      - Follow the **Main Spec Format Reference** below

5. **Validate updated main specs**

   Run `speccraft validate --specs` with the same selected-root flags used earlier.
...
- A MODIFIED block carries the whole requirement - body plus every scenario that survives the change. `speccraft validate` and `speccraft archive` both reject one that drops a scenario the main spec still has.

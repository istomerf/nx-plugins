---
name: speccraft-full
description: Run the full SpecCraft cycle end-to-end: investigate, propose (fast path), pause for a single plan-approval gate, then implement, verify, and auto-fix whatever verify finds. Use when the user wants the whole idea-to-verified-code cycle done in one pass with just one checkpoint before code changes.
...
**This workflow is autonomous apart from one approval gate**: unlike `/speccraft-propose` (which stops after planning and waits indefinitely) or a plain `/speccraft-apply-change` (which can pause on any ambiguity), `/speccraft-full` chains propose's fast path, a single plan-approval gate, apply, verify, and an automatic fix loop into one run. Once you approve the plan, nothing else asks for confirmation - except a genuine blocker `/speccraft-apply-change` cannot resolve on its own, which still stops the whole run rather than being papered over.
...
1. **Resolve the change and check for a resumable run**
   ...
   - If the change exists but has no `flow` field: this change was not started by `/speccraft-full` - report that plainly and stop rather than guessing where an unrelated change stands.

2. **Investigate (explore stance, non-interactive)**

   Before creating anything, briefly investigate the problem and the codebase the way `/speccraft-explore` does: read relevant files, note existing conventions, integration points, and constraints. Do not ask the user questions here and do not create any artifacts yet - this is discovery to inform propose, not a conversation.
...
4. **Propose (fast path)**
   ...
   Run `/speccraft-propose`'s fast path inline for this change: create every artifact the schema's apply phase transitively depends on, using the configured default schema, making reasonable assumptions instead of asking wherever `/speccraft-propose`'s own instructions would otherwise pause to ask (per its "Fast path" step) - grounded in step 2's investigation, not invented from nothing.
...
6. **Apply**
   ...
   Run `/speccraft-apply-change`'s instructions inline for this change: implement every task until done or blocked.
   - If apply reports a blocked state, or pauses on a genuine ambiguity or error it cannot resolve on its own: STOP the whole `/speccraft-full` run here. Update the flow state to `phase: Blocked`, report exactly what blocked it, and wait for the user. Do not guess past an apply-level blocker or silently narrow scope to route around it.
   - Once every task is done: update the flow state to `phase: Verify`, append `{ phase: Apply, at: <now> }`.

7. **Verify**

   Run `/speccraft-verify-change`'s instructions inline for this change and capture its CRITICAL / WARNING / SUGGESTION findings and overall verdict. Update the flow state's `lastVerifyVerdict` to that verdict.
...
9. **Final report**
   ...
   - Suggest `/speccraft-archive-change` as the next step - this workflow never archives automatically
...
- Never override `/speccraft-apply-change`'s own guardrails: a genuine blocker there stops `/speccraft-full` entirely, it is never auto-resolved or skipped past.
...
- This workflow never runs `/speccraft-archive-change` itself - archiving remains an explicit, separate action the user takes afterward.

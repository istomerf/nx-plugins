...
1. **Select the change**
   ...
   Always announce: "Using change: <name>" and how to override (e.g., `/speccraft-archive-change <other>`).
...
5. **Verify before archiving — blocks on CRITICAL findings**
   ...
   If `/speccraft-verify-change` (or the `speccraft-verify-change` workflow) already ran earlier in this
   conversation for this exact change and reported zero unresolved CRITICAL issues, reuse
   that result and skip re-running it.

   Otherwise, run the `speccraft-verify-change` workflow inline for `<name>` (agent-driven;
   reuse the `status`/`instructions` data already loaded in steps 2-3 instead of re-fetching
   it) and capture its CRITICAL/WARNING/SUGGESTION findings.
...
**Specs:** <"✓ Synced to main specs" only if the step 4 verification passed; "No delta specs"; or "Archived without syncing (explicit override)">
**Verify:** <"✓ No critical issues" / "Archived over N critical issue(s) — explicit override" / "Skipped — already verified clean earlier in this conversation">
...
- If sync is requested, run the `speccraft-sync-specs` workflow inline (agent-driven)
